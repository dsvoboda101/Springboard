...
/* I'm unclear on the question; if the solution is no duplicate data on the combination of member and tennis court, here is my query */
...
/* if the solution is no duplicate members even if they have booked both Court 1 and Court 2, here is my query */

SELECT CONCAT(m.firstname,' ', m.surname) AS member, MIN(f.name) AS court
FROM Bookings AS b
LEFT JOIN Members AS m ON m.memid = b.memid 
LEFT JOIN Facilities as f ON b.facid = f.facid 
WHERE b.facid IN (0, 1)
...
SELECT 
    ...
    guestcost AS cost
FROM
(SELECT m.firstname, m.surname, f.name, f.membercost, f.guestcost, b.facid, b.memid, b.starttime
FROM Facilities AS f
JOIN Bookings as b ON b.facid = f.facid
JOIN Members as m ON b.memid = m.memid
WHERE b.starttime LIKE '2012-09-14%'
    AND (f.membercost >=30 OR f.guestcost >= 30)) AS sub
ORDER BY cost DESC;
